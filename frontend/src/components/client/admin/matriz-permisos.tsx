...
import { asignarPermisosRolAction } from '@/app/(admin-plaza)/admin/usuarios-plaza/permisos/actions';
import { confirmAction } from '@/lib/sweetalert';
import { useContainerWidth } from '@/components/client/responsive/use-container-width';
 ...
 *
 * Responsive (2026-09-16): la matriz necesita ~220 px + 140 px por rol. En
 * contenedores estrechos (< `COMPACT_MAX_WIDTH`, teléfonos y tablets en
 * vertical) se muestra el modo compacto: un selector de rol y, por módulo,
 * una lista de permisos con un checkbox por fila. El estado (`draft`,
 * `isDirty`, guardar/descartar) es el mismo en ambos modos.
 *
...
/** Por debajo de este ancho de contenedor (px) se usa el modo compacto. */
...
export function MatrizPermisos({
  ...
  const compact = containerWidth !== null && containerWidth < COMPACT_MAX_WIDTH;
  /** Rol activo en modo compacto (por defecto, el primero editable). */
  ...
  // ── Modo compacto: un rol a la vez ──
  const rolActivo =
    roles.find((r) => r.id === rolActivoId) ??
    roles.find((r) => !rolesSistemaIds.has(r.id)) ??
    roles[0];
  const compactView = rolActivo ? (
    <>
      <Card pad>
        <div className="matriz-rol-picker">
          <label htmlFor="matriz-rol" className="text-sm font-medium">
            Rol a editar
          </label>
          <select
            id="matriz-rol"
            className="select"
            value={rolActivo.id}
            onChange={(e) => setRolActivoId(e.target.value)}
          >
            {roles.map((r) => (
              <option key={r.id} value={r.id}>
                {r.nombre}
                {rolesSistemaIds.has(r.id) ? ' (sistema)' : ''}
              </option>
            ))}
          </select>
          {rolesSistemaIds.has(rolActivo.id) ? (
            <p className="muted text-xs">
              El rol del sistema es inamovible: siempre tiene todos los permisos.
            </p>
          ) : (
            isDirty(rolActivo.id) && (
              <span className="badge b-warn" style={{ alignSelf: 'flex-start' }}>
                <span className="bdot" />
                Cambios sin guardar
              </span>
            )
      ...
      {permisosPorModulo.map((m) => {
        const isSistema = rolesSistemaIds.has(rolActivo.id);
        const set = draft.get(rolActivo.id) ?? new Set<string>();
        const ids = m.permisos.map((p) => p.id);
        const allChecked = ids.every((id) => set.has(id));
        const someChecked = !allChecked && ids.some((id) => set.has(id));
        return (
          <Card key={m.modulo}>
            <div className="card-head" style={{ justifyContent: 'space-between' }}>
              <div>
                <div className="card-title">{m.modulo}</div>
                <div className="muted text-xs">{m.permisos.length} permiso(s)</div>
              </div>
              {!isSistema && (
                <label className="muted text-xs" style={{ cursor: 'pointer', flex: 'none' }}>
                  <input
                    type="checkbox"
                    checked={allChecked}
                    ref={(el) => {
                    ...
                    onChange={(e) => toggleAll(rolActivo.id, m.permisos, e.target.checked)}
                    style={{ marginRight: 4 }}
                  />
                  todos
                </label>
            ...
            <div className="card-body" style={{ paddingTop: 4, paddingBottom: 4 }}>
              <ul className="matriz-list">
                {m.permisos.map((p) => {
                  const inputId = `mp-${rolActivo.id}-${p.id}`;
                  return (
                    <li key={p.id}>
                      ...
                      <input
                        ...
                        disabled={isSistema}
                        onChange={(e) => togglePermiso(rolActivo.id, p.id, e.target.checked)}
                        aria-label={`${p.codigo} → ${rolActivo.nombre}`}
                      />
                    </li>
                  );
                })}
    ...
    <div ref={wrapRef} className="space-y-3">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <p className="muted text-sm">
          {catalogo.total} permisos · {roles.length} rol(es) ·{' '}
          {roles.filter((r) => rolesSistemaIds.has(r.id)).length} de sistema
        </p>
      </div>
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    {roles.map((r) => {
                      ...
                      const someChecked =
                        !allChecked &&
                        moduloPermIds.some((id) => (draft.get(r.id) ?? new Set()).has(id));
                      return (
                        <th
                          key={r.id}
                          style={{
                        ...
                        >
                          ...
                          >
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                              {isSistema && <Lock size={12} aria-label="Rol del sistema" />}
                              <span>{r.nombre}</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                              {isSistema ? (
                                ...
                                <label className="muted text-xs" style={{ cursor: 'pointer' }}>
                                  <input
                                    ...
                                    onChange={(e) => toggleAll(r.id, m.permisos, e.target.checked)}
                                    style={{ marginRight: 4 }}
                                  />
                                  todos
                                </label>
                              ...
                              {dirty && !isSistema && (
                                <span className="badge b-warn" title="Cambios sin guardar">
                                  <span className="bdot" />
                                </span>
                              )}
                            </div>
      ...
      <Card pad>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="muted text-sm">
            {roles.filter((r) => !rolesSistemaIds.has(r.id) && isDirty(r.id)).length} rol(es) con
            cambios pendientes
          </div>

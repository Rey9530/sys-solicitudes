...
- **Bitácora de cambios:**
  ...
  - **actualización 2026-06-08 — Refactor SMTP/Nodemailer → Mailgun API HTTP (a petición del owner):** el transporte ya NO usa Nodemailer; envía por la API REST de Mailgun con `mailgun.js@13.2.0` + `form-data@4.0.5` (versiones latest verificadas en el registry). Patrón tomado de un servicio de referencia de otro proyecto: `new Mailgun(FormData).client({ username: 'api', key, url }).messages.create(domain, { from, to, subject, html })`. La firma `MailerService.send(to, subject, html)` y la clase `MailerSendError` (`esHardBounce`/`responseCode`) se conservan intactas → el worker (T-122), `EmailService` (T-121), `TemplateRendererService` (T-120) y el wrapper de auth (T-126) NO cambiaron.
    - **Variables `.env`:** se reemplazan `SMTP_HOST/PORT/SECURE/USER/PASSWORD/FROM` por `MAILGUN_API_KEY`, `MAILGUN_DOMAIN`, `MAILGUN_FROM`, `MAILGUN_API_URL` (default US `https://api.mailgun.net`; EU `https://api.eu.mailgun.net`). Actualizados `.env.example` (raíz + backend) y `.env` reales; `MailerModule.forRootAsync` en `app.module.ts` ajustado.
    - **Modo dev log-only:** sin `MAILGUN_API_KEY` el `MailerService` NO crea el cliente (mailgun.js lanza `"Parameter key is required"` con key vacía) y `send()` solo loggea `to`+`subject` sin enviar → reemplaza la necesidad de MailHog en local. MailHog queda sin uso en `docker-compose.yml` (contenedor conservado, no utilizado para correo).
    - **Clasificación de errores adaptada** a la API de Mailgun (campo `status`): `401`→`invalid_auth`; red (`ETIMEDOUT/ECONNRESET/ECONNECTION/ESOCKET`) o `>=500`/`429`→`connection_timeout` (reintentable); `400`→`recipients_refused`; resto→`unknown`. Se mantiene el mapeo `hard_bounce` (550/551/553) por compatibilidad.
    - ⚠️ **Cambio de comportamiento (bounce):** con la API de Mailgun `messages.create` responde 200 y ENCOLA el mensaje; los hard bounce reales llegan después por webhooks/Events API, NO síncronos. Por tanto el `hard_bounce` síncrono de T-124 (que marca `usuario.email_invalido=true`) prácticamente ya no se dispara. El marcado por bounce real vía webhook de Mailgun queda **fuera de alcance** de este refactor (futuro: endpoint `/api/v1/notificaciones/webhook` de Mailgun).
    - **Deps:** removidas `nodemailer` (dep) y `@types/nodemailer` (devDep) — único consumidor era este servicio. Verificado: `npm run type-check` y `npm run build` en verde; boot del backend supera la init del `MailerService` en modo log-only sin errores.

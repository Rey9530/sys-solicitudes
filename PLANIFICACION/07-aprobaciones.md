...
- **Bitácora de cambios:**
  ...
  - **actualización 2026-08-11 (fix admin del sistema):** El FE envía `?asignadasAMi=true` por defecto. Para usuarios con `rol_staff.es_sistema=true` ("Administrador del sistema") ese filtro esconde las solicitudes en `enviada` (sin `admin_asignado_id` aún), que **deben** aparecer en su bandeja. Se añade helper privado `actorEsAdminDelSistema(tx, actor)` que resuelve el flag dentro del `withTenant`; cuando es `true` el filtro `asignadasAMi` se omite. **Decisión:** la excepción se aplica solo en backend (T-V03: regla transversal, no debe filtrarse por quién la consume). ⚠️ **Root cause de un 500 encontrado durante el fix:** la query de `rol_staff` debe ejecutarse **dentro** de `withTenant`. Si corre antes, la RLS evalúa `current_setting('app.plaza_id')::uuid` con el GUC aún sin setear y Postgres responde `invalid input syntax for type uuid: ""` (HTTP 500). Se valida con UUID hardcoded que el bug NO es del argumento ni del cliente Prisma, sino del orden de operaciones. La regex UUID se mantiene como defensa para JWTs degenerados (inquilinos / superadmin / tokens viejos). Verificación E2E: con `admin@demo.com` (`es_sistema=true`), `?asignadasAMi=true` devuelve `total=2` incluyendo una solicitud en `enviada` con `adminAsignadoId=NULL`; al pasar el rol a `es_sistema=false` el filtro aplica correctamente y solo ve la solicitud asignada a él. Logs de pino limpios (sin `DBG-*` ni errores). Archivos: `backend/src/modules/aprobaciones/aprobaciones.service.ts:303-374` (bandeja), `:486-505` (UUID_RE) y `:506-516` (actorEsAdminDelSistema). Tarea relacionada abierta: **#20** — crear migración `GRANT SELECT ON rol_staff TO syssol_app` para que el lookup siga funcionando tras `prisma migrate reset` (hoy requiere workaround manual con `docker exec`).

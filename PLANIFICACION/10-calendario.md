...
- **Bitácora de cambios:**
  ...
  - ⚠️ **2026-09-24 — fix feed inquilino (bug reportado: "aparecen 2 eventos" y "evento pedido a las 15:42 sale a las 09:42").** Causa raíz en `calendario.service.ts`: (1) `solicitudesCalendario` emitía TODA solicitud con `fecha_evento_inicio` (incluidas `aprobada`/`cerrada`), por lo que una solicitud aprobada llegaba dos veces al inquilino (`evt-…` desde `evento_calendario` y `sol-…` desde `solicitud`); (2) `combinarFechaHora` hacía `setUTCHours(HH,MM)` sin aplicar `PLAZA_UTC_OFFSET_MS`: la hora de la plaza salía etiquetada como UTC (`15:42Z` → 09:42 en America/El_Salvador). Corrección: `combinarFechaHora` suma el offset (mismo criterio que `AprobacionesService.combinar`); `feed()` descarta los items `solicitud` cuyo `solicitudId` ya está como item `evento` (dedup en memoria, así con `?tipo=solicitud` solo la solicitud aprobada sigue visible con hora correcta); el rango de `solicitudesCalendario` se compara en días civiles de la plaza (`fechaCivilPlaza(from/to)`) con solapamiento `[inicio, fin]` (antes `gte: from` contra un `DATE` perdía los eventos del primer día visible). El admin NO estaba afectado (su feed nunca incluye `solicitud`). Verificado con suite e2e Playwright `frontend/e2e/calendario.spec.ts` (`@playwright/test` **1.63.0**, latest en registry; rojo antes del fix → verde después) y con `curl` directo al feed (`start` = `…T21:42:00.000Z` para 15:42 plaza; plaza `qa-b` no ve eventos de `demo`).

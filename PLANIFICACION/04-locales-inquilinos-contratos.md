...
- **Bitácora de cambios:**
  ...
  - 2026-06-10: **T-059-bis — Pestaña «Usuarios» en el detalle de inquilino.** Se añadió un 4º tab `Usuarios` con `count`, alimentado por el nuevo endpoint anidado `GET /inquilinos/:id/usuarios` (`UsuariosService.findByInquilino` con paginación/filtros `rol`/`search`/`activo`). Backend extendido con `PATCH /usuarios/:id` (nombre/teléfono), `DELETE /usuarios/:id` (soft delete, solo rol `inquilino`, idempotente con `409 USUARIO_YA_INACTIVO`, revoca `refresh_token` activos), `POST /usuarios/:id/reset-password` (admin dispara `MailerService.sendPasswordReset` con token de 1h, `409 USUARIO_EMAIL_INVALIDO` si hay hard bounce) y `POST /usuarios/:id/reactivate` (`409 USUARIO_NO_INACTIVO` si ya estaba activo). Cada acción registra en `auditoria` con `accion: 'usuario.{disable,update,admin_password_reset,reactivate}'`. Frontend: nuevos `usuarios-inquilino-table.tsx` (tabla con badges Activo/Inactivo/Email inválido) y `editar-usuario-inquilino-dialog.tsx`; 4 server actions nuevas en `inquilinos/actions.ts`. `tsc --noEmit` y `npm run lint` limpios.
  - 2026-06-10: ⚠️ **Decisión vinculante nueva — SweetAlert2 obligatorio para diálogos de decisión.** Refactor de los `if (!confirm(...)) return;` que quedaban en el frontend (`adjunto-uploader.tsx`, `categorias-table.tsx`, `editar-inquilino-form.tsx`, `inquilinos-table.tsx`, `locales-table.tsx`, `plazas-table.tsx`, `subcategorias-manager.tsx` y la nueva `usuarios-inquilino-table.tsx`) a `confirmAction({...})` desde el nuevo wrapper `frontend/src/lib/sweetalert.ts` (SweetAlert2 v11.26.25, instalada vía `npm install sweetalert2@^11.26.25`). Razón: look & feel consistente, ARIA / focus trap correctos, mockeable para tests futuros, evita warnings de Next.js con Server Components. Toasts efímeros NO modales siguen por `sonner`. Auditoría rápida: `Get-ChildItem -Path "frontend\src" -Recurse -Include "*.ts","*.tsx" | Select-String -Pattern '\bconfirm\(|\bwindow\.alert\b'` debe devolver 0 matches fuera del wrapper. **Regla documentada en `docs/07-arquitectura.md` §7.4.5 y `docs/02-stack-tecnologico.md` §2.10; regla operativa en `CLAUDE.md` bajo «Reglas operativas para implementación».** Aplica a TODA acción destructiva o de decisión que se añada en módulos futuros (cancelar solicitud, cerrar contrato, etc.).
